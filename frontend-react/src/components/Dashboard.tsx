...
function DonutChart({ slices, empty }: { slices: PieSlice[]; empty: boolean }) {
  ...
  slices.forEach((slice, i) => {
    ...
    const xi2 = cx + inner * Math.cos(startAngle), yi2 = cy + inner * Math.sin(startAngle);
    const large = angle > Math.PI ? 1 : 0;
    const d = `M${x1} ${y1} A${r} ${r} 0 ${large} 1 ${x2} ${y2} L${xi1} ${yi1} A${inner} ${inner} 0 ${large} 0 ${xi2} ${yi2}Z`;
    paths.push(<path key={i} d={d} fill={empty ? '#e2e8f0' : slice.color} />);
    startAngle = end;
  });
  return (
...
export default function Dashboard() {
  ...
  const fmtDur = (min: number) => { const h = Math.floor(min / 60), m = min % 60; return `${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}:00`; };
  ...
  const pieData: PieSlice[] = catEntries.map(([name, time]) => {
    ...
    return { name, value: sec, pct: totalCatSec > 0 ? Math.round((sec / totalCatSec) * 100) : 0, color: CATEGORY_COLORS[name] ?? '#cbd5e1' };
  });

  const noCatData = pieData.length === 0;
  const displayPie: PieSlice[] = noCatData
    ? [{ name: '開発', value: 1, pct: 45, color: '#e2e8f0' }, { name: '学習', value: 1, pct: 25, color: '#e2e8f0' }, { name: 'その他', value: 1, pct: 30, color: '#e2e8f0' }]
    : pieData;
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="col-span-2 p-6" style={CARD}>
          <div className="flex items-start justify-between mb-5">
            ...
            <span className="px-2.5 py-1 text-[11px] font-bold bg-emerald-500 text-white rounded-lg tracking-wide">FOCUS MODE</span>
          </div>
          <h2 className="text-4xl font-bold text-slate-900 mb-1 tracking-tight">{data?.current_app ?? '—'}</h2>
          {data?.current_domain && <p className="text-sm text-slate-400 mb-5">{data.current_domain}</p>}
          {!data?.current_domain && <div className="mb-5" />}
          <div>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="p-5 flex-1" style={CARD}>
            <div className="flex items-center justify-between mb-1.5">
              <p className="text-[11px] font-medium text-slate-400">コンテキストスイッチ</p>
      ...
      <div className="grid grid-cols-2 gap-4">
            ...
            <div className="space-y-2.5 flex-1">
              {noCatData ? (
                <div className="text-[12px] text-slate-400 leading-relaxed">
                  ...
                  <p>Windowsトラッカーが起動すると<br />カテゴリが表示されます</p>
                </div>
              ) : (
                displayPie.map(e => (
        ...
        <div className="p-5" style={CARD}>
          <div className="flex items-center justify-between mb-4">
            <p className="text-[13px] font-semibold text-slate-800">頻繁な遷移パターン</p>
                ...
                <div key={i}>
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center space-x-1.5 text-[13px] text-slate-700 min-w-0">
                      <span className="truncate max-w-[90px]">{t.from}</span>
                      <span className="text-slate-300 flex-shrink-0">→</span>
                      <span className="truncate max-w-[90px]">{t.to}</span>
                  ...
                  <div className="h-1.5 bg-slate-100 rounded-full overflow-hidden">
                    <div className="h-1.5 bg-blue-500 rounded-full transition-all" style={{ width: `${(t.count / maxCount) * 100}%` }} />
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="p-5" style={CARD}>
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center space-x-4 text-[11px] text-slate-400">
            ...
            <span className="flex items-center space-x-1.5"><span className="w-2.5 h-2.5 rounded-sm bg-blue-200 inline-block" /><span>休憩</span></span>
          </div>
        </div>
        {!hasActivity ? (
